# CPC200-CCPA Firmware Microphone Processing

## Overview
CPC200-CCPA firmware processes microphone audio from external sources (Autokit app) and forwards to CarPlay/Android Auto protocols. A15W model lacks onboard microphone, implementing bidirectional audio bridging via USB.
...
## Hardware Context
**A15W Variant**: No onboard microphone, receives data from host apps via USB NCM interface
**Protocols**: CarPlay, Android Auto, Android Mirror, iOS Mirror, HiCar
**Config**: `MDLINK='CarPlay,AndroidAuto,AndroidMirror,iOSMirror,HiCar'`

## Data Flow Pipeline
```
Autokit App → USB NCM → boxNetworkService → MicAudioProcessor → 
...
## Processing Pipeline
1. **USB Reception**: Autokit app → USB NCM → boxNetworkService
...
**Optimizations**: 200-byte VR packets, direct sample rate conversion, hardware-accelerated RTP, USB NCM bulk transfer
...
### Testing
- Voice packets: `VrPacketLen: 200`, `VrVolumGain: 0`
- Background: `IsSupportBGRecord()`
